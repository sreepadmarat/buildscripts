#!/bin/bash                                                                                                                                                                                   
    ...
    (cd packages/apps/Settings 2>/dev/null && git cherry-pick --abort 2>/dev/null || true)                                                                                                        
    ...
    <manifest>                                                                                                                                                                                    
      ...
      <project path="kernel/oneplus/sm6375" name="sreepadmarat/android_kernel_oneplus_sm6375" remote="github" revision="lineage-23.2" clone-depth="1" />                                 
    ...
    # --- ReVanced Cherry-Picks (Safely handled if conflict occurs) ---
    echo "Applying ReVanced framework patches..."
    cd frameworks/base
    git fetch https://github.com/PixelLineage/frameworks_base.git bc71449a25b6b0d16d2b4e611cdc9939bd89bb54 && git cherry-pick FETCH_HEAD || git cherry-pick --abort
    cd ../..
  
    cd packages/apps/Settings
    git fetch https://github.com/PixelLineage/packages_apps_Settings.git 38def62c73e175bf65708137c3e9e281c476ba84 && git cherry-pick FETCH_HEAD || git cherry-pick --abort
    cd ../../..
    ...
    cp out/target/product/larry/lineage-23.2*.zip \
       ...
       out/target/product/larry/dtbo.img los/ 2>/dev/null || true
